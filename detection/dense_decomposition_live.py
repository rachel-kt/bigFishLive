...
def decompose_dense_live(
    ...
    """
    #print(spots.size)
    reference_spot = deepcopy(reference_spot_previous)
    # check parameters
    stack.check_array(
        image,
        ndim=[2, 3],
    ...
    if spots.size == 0:
        ...
        if reference_spot_previous.size==0:
    ...
    regions_to_decompose, spots_out_regions, region_size = get_dense_region_live(
        image=image_denoised,
        spots=spots,
        voxel_size=voxel_size,
        spot_radius=spot_radius,
...
def get_dense_region_live(image, spots, voxel_size, spot_radius, reference_spot, beta=1):
...
#        spot_radius=spot_radius,
#        alpha=0.5)
    median_spot = deepcopy(reference_spot)
